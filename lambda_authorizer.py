...
def lambda_handler(event, context):
    ...
    associated with the request. This can be accomplished in a number of ways:
    ...
    If the request is valid, a policy must be generated which will allow or deny
    ...
    '''
    principalId = event['queryStringParameters']['orgid']
    certificate_cn_parse = re.match(
        r'^.*CN=(?P<CN>[^,]+).*$', event['requestContext']['identity']['clientCert']['subjectDN'])
    #context_response = {}
    ...
    ddb_response = map_uuid_to_tenant(certificate_cn_parse['CN'])
    org_id_from_ddb = ddb_response['Item']['orgid']['S']
    if org_id_from_ddb == principalId:
    ...
    #authResponse['context'] = context_response

    return authResponse


def map_uuid_to_tenant(apid):
    table = 'CustomerTable'
